...
#文件修改，sed简单替换
import sys
f_read = open("lyrics","r",encoding="utf-8")            #文件修改的思路：即将修改好的文件保存至另一份文件中
f_write = open("file1","w",encoding="utf-8")
find_str = sys.argv[1]                              #表示程序执行时所带的第一个参数
replace_str = sys.argv[2]                           #表示程序执行时，所带的第二个参数
for line in f_read:
    if find_str in line:
        line = line.replace(find_str,replace_str)   #字符串内容替换
    f_write.write(line)

f_read.close()
f_write.close()

# 使用with打开，不需要关闭，with执行完，文件关闭
with open("lyrics","r") as file_1,open("file1","w") as file_2:      #同时打开多个文件
    print(file_1)
'''
